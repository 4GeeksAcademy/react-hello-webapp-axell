...
export const AddContact = () => {
  ...
  const handleSubmit = async e => {
    e.preventDefault();
    if (!form.name || !form.email || !form.phone || !form.address) return alert("Todos los campos son obligatorios");
    ...
    try {
      if (id) {
        ...
        dispatch({ type: "ADD_CONTACT", payload: newContact.contacts ? newContact.contacts[0] : newContact });
      }
      navigate("/");
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="container mt-4">
      ...
      <form onSubmit={handleSubmit}>
        <input type="text" name="name" placeholder="Nombre completo" className="form-control mb-2" value={form.name} onChange={handleChange} required />
